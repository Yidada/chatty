...
| 设置 | 工作区、Runtimes、Agents、Squads | 可取消的工作区选择面板/退出；原生资源列表和完整详情页。 |
...
- 每个 Tab 在当前运行中保留页面层级、搜索/筛选、已加载分页和阅读位置；服务端数据仍仅驻留内存。
- Issue 和资源详情使用完整页面，顶部返回与系统返回均逐级返回；隐藏的 Tab 不接收返回操作。
- 工作区选择支持取消、系统返回和当前工作区选中标记；成功选定另一工作区后才替换当前页面作用域。
- 实现及生命周期边界见 `docs/android-navigation-continuity.md`。
