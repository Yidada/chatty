...
日期：2026-09-13。用户明确交付包含 Git 代码推送和独立 Chatty Next 的 TestFlight 内测。当前状态：`Next` 已推送至 GitHub；Bundle ID 已注册、分发签名 IPA 已导出，等待 Apple 网页登录以创建应用记录；尚未上传至 Apple。
...
4. 已执行 `git push --set-upstream origin Next`，远端分支创建成功，包含代码提交 `6af5189` 及发布准备记录。`main` 保持 `7f4703e`；未创建 PR 或合并主分支。
...
用户已授权代码推送与独立 TestFlight 内测。API 认证有效；CLI 网页 session 与浏览器登录均已过期。已将 Ego Lite 的本次发布窗口实际切到前台，并交给用户完成登录和双重验证。Apple 页面仍显示空密码框；认证完成后在同一窗口创建应用，并继续上传、处理与内部组分配。无需再次确认发布目标。
