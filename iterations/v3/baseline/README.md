# CLE-73 Stage 2 baseline harness

1A–6A were approved on 2026-09-06. This change prepares measurement; it does not optimize the product or freeze budgets. See `../intent.md` and `pr-audit.md`. Device measurements and omissions belong in `results.md`.
...
- S2: 20 sessions, 1,000 messages in target s1, 20% Markdown, 10 image metadata entries, active pending task, 200 traces. Only s1 has history. Pagination includes tied timestamps and uses the `(created_at,id)` cursor contract. The current scroll benchmark exercises the loaded window; it does **not** load all 20 pages or inject all 200 live events.
- S3: 50 projects, 2,000 issues, 34 agents + 33 runtimes + 33 squads. Current startup/scroll test is Chat, not S3 resource traversal; S3 dataset availability alone is not UI coverage.
- S4: S2 plus `POST /__perf` controls: `delay_ms`, `status`, `retry_after`, `disconnect`, `drop_ws`, ordered `events` (caller may intentionally reorder). A delay above the unchanged 25s call timeout exercises timeout. No automatic fault assertions are claimed by a successful fixture unit test.
