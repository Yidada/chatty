...
- `README.md` "Status" section updated in this PR to point at Stage 2 and this spec, per acceptance criteria — no other content in `README.md` was rewritten (see PR notes: `README.md`'s "Core Principles" section still describes the pre-pivot Lark/Context-Layer vision from `iterations/v1/`; that section is now materially out of sync with `iterations/v2/intent.md`'s explicit exclusion of Lark from V1 scope, but rewriting it is a separate content decision outside this Issue's ask — flagged here, not silently left inconsistent).
