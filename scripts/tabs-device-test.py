...
try:
 ...
 before=json.load(urllib.request.urlopen(FIXTURE+'/__calls'));time.sleep(3);after=json.load(urllib.request.urlopen(FIXTURE+'/__calls'));assert len(after['calls'])-len(before['calls'])<3,'Unexpected repeated loading'
 s.tap('Loop Project');s.expect('共 55 个 · 已加载 50 个');s.tap('Issue 01');s.expect('Synthetic issue description');s.tap('状态：待开始');s.tap('内部验收');s.expect('状态：内部验收');s.shot('status-updated');s.tap('返回 Issues')
 ...
 s.tap('返回项目');s.tap('Empty Project');s.expect('没有符合条件的 Issue。');s.tap('返回项目');s.tap('未归属项目的 Issues');s.expect('Unassigned project issue');s.shot('no-project');s.tap('返回项目')
 s.tap('设置');s.expect('切换工作区');s.expect('退出登录');s.expect('Runtimes');s.expect('Agents');s.expect('Squads');s.shot('settings')
 s.tap('Runtimes');s.expect('Loop Runtime');s.tap('Loop Runtime');s.expect('在线');s.shot('runtime-detail');s.tap('返回 Runtimes');s.tap('返回设置')
 s.tap('Agents');s.expect('Mika Renamed');s.tap('Mika Renamed');s.shot('agent-detail');s.tap('返回 Agents');s.tap('返回设置')
 s.tap('Squads');s.expect('Loop Squad');s.tap('Loop Squad');s.expect('3 位成员');s.shot('squad-detail');s.tap('返回 Squads');s.tap('返回设置')
 for _ in range(3):
