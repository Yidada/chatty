...
try:
 ...
 s.tap('Loop Test Workspace');s.expect('Mika');s.shot('connected')
 ...
 restart();s.expect('Mika');s.shot('cold-start-restored')
 s.tap('设置');s.tap('切换工作区');control(503);s.tap('重新加载')
 s.expect('暂时无法连接，请重试。已有登录信息会保留。');s.shot('server-503')
 restart();s.expect('暂时无法连接，请重试。已有登录信息会保留。')
 control(200);s.tap('重新加载');s.expect('Mika');s.shot('recovered')
 s.tap('设置');s.tap('切换工作区');control(401);s.tap('重新加载')
