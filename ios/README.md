...
- 输入框是 `ios/Chatty/ComposerText.swift` 里的 `UITextView`：竖排 `TextField` 无法表达「回车即发送」（`axis: .vertical` 只插入换行，既不回调 `onSubmit` 也不接受 `.submitLabel(.send)`）。回车规则集中在 `ComposerReturnKey`，宿主 XCTest 用它覆盖组词上屏与八行封顶。
