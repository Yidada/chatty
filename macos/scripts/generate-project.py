...
root=Path(__file__).resolve().parents[1]; objects={}
...
package=obj('package','XCLocalSwiftPackageReference',relativePath='../ios/Packages/ChattyKit')
productrefs=[];targets=[]
for name in ('Chatty','ChattyFixture'):
 ...
 settings={'PRODUCT_NAME':'$(TARGET_NAME)','PRODUCT_BUNDLE_IDENTIFIER':'ai.chatty.macos'+('.fixture.tests' if test else '.fixture' if fixture else ''),'CODE_SIGN_STYLE':'Automatic','SUPPORTED_PLATFORMS':'macosx','MACOSX_DEPLOYMENT_TARGET':'15.0','ENABLE_HARDENED_RUNTIME':'YES','CODE_SIGN_ENTITLEMENTS':'Config/Chatty.entitlements','SWIFT_VERSION':'6.0','MACOSX_DEPLOYMENT_TARGET':'15.0','LD_RUNPATH_SEARCH_PATHS':['$(inherited)','@executable_path/../Frameworks'],'MARKETING_VERSION':'0.1.0','CURRENT_PROJECT_VERSION':'1'}
...
project=obj('project','PBXProject',attributes={'BuildIndependentTargetsInParallel':'YES','LastUpgradeCheck':'2660'},buildConfigurationList=cfgs('project',{'SDKROOT':'macosx','SWIFT_VERSION':'6.0','SWIFT_STRICT_CONCURRENCY':'complete','CLANG_ENABLE_MODULES':'YES','MACOSX_DEPLOYMENT_TARGET':'15.0'}),compatibilityVersion='Xcode 14.0',developmentRegion='en',hasScannedForEncodings='0',knownRegions=['en','Base','zh-Hans'],mainGroup=main,packageReferences=[package],productRefGroup=products,projectDirPath='',projectRoot='',targets=targets)
...
for name in ('Chatty','ChattyFixture'):
 info={'CFBundleDevelopmentRegion':'$(DEVELOPMENT_LANGUAGE)','CFBundleExecutable':'$(EXECUTABLE_NAME)','CFBundleIdentifier':'$(PRODUCT_BUNDLE_IDENTIFIER)','CFBundleInfoDictionaryVersion':'6.0','CFBundleName':'$(PRODUCT_NAME)','CFBundleDisplayName':'Chatty Test' if name.endswith('Fixture') else 'Chatty','CFBundlePackageType':'APPL','CFBundleShortVersionString':'$(MARKETING_VERSION)','CFBundleVersion':'$(CURRENT_PROJECT_VERSION)','LSMinimumSystemVersion':'$(MACOSX_DEPLOYMENT_TARGET)','NSHighResolutionCapable':True,'NSPrincipalClass':'NSApplication','NSHumanReadableCopyright':'Copyright © 2026 Chatty','NSApplicationSupportsSecureRestorableState':True}
 if name.endswith('Fixture'): info['NSAppTransportSecurity']={'NSAllowsLocalNetworking':True}
