...
struct AppBootstrap: View {
    ...
    private func prepare() {
        ...
        do {
            let identity = Bundle.main.bundleIdentifier ?? "ai.chatty.macos"
    ...
    var body: some View {
        NavigationStack {
            ScrollView {
                    ...
                    VStack(alignment: .leading, spacing: 14) {
                        if model.codeSent {
                            ...
                            TextField("六位验证码", text: $model.code)
                                .textContentType(.oneTimeCode).focused($focused)
                            ...
                            TextField("邮箱地址", text: $model.email)
                                .textContentType(.emailAddress).focused($focused).autocorrectionDisabled()
                ...
                }.padding(40).frame(maxWidth: 560).frame(maxWidth: .infinity).padding(.top, 60)
            }
...
struct OfflineDraftView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            }.background(ChattyTheme.background).navigationTitle("本机草稿")
